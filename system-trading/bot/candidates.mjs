/**
 * 후보 전방 검증 러너 — 기획 10선 검증(2026-08-17) 회차의 신규 후보 2개를
 * 본대(쿼드)와 분리된 페이퍼 북("cand")에서 검증한다.
 *
 *   node system-trading/bot/candidates.mjs          # 1회 실행
 *   node system-trading/bot/candidates.mjs --loop   # 4H 마감마다 자동
 *
 * 왜 run.mjs 에 합치지 않았나:
 ...
 *
 * 세 번째 후보(4H 골든크로스 1:3)는 이미 쿼드의 gc 로 라이브 가동 중 — 그 전방
 * 검증은 라이브 북이 맡는다. 여기는 신규 2개만 담는다.
 *
 * 판정·체결 규칙은 백테스트(scripts/backtest/ten-strategies.mjs)와 동일:
 * 마감 봉 판정 → 다음 봉 시가(≈마감 직후 현재가) 진입, 손절 우선, 갭이면 시가 체결.
 * 일봉 필터는 신호 봉 시가 이전에 마감 완료된 일봉만 본다(진행 중 일봉은 미래 참조).
 * 리스크는 승격 사다리 첫 칸인 2%: L = min(10, 2% ÷ (손절폭% + 0.1%)).
 */
...
const CFG = {
  ...
  /** 스펙 동결 2026-08-17 — 기획 10선 검증 회차의 게이트 통과 조합 그대로. */
  members: {
    dch: {
      tf: "4H",
      name: "신저가 이탈 숏 + 일봉 하락",
      ...
      rule: "종가가 직전 20봉 최저가 아래로 마감 + 일봉 종가 < 일봉 SMA50",
    },
    mcv: {
      tf: "4H",
      name: "MACD 교차 + 거래량 확장",
      ...
      rule: "MACD(12,26,9) 라인이 시그널 상향 교차 마감 + 거래량 ≥ 1.5×20봉 평균",
    },
  },
  riskPct: 2,
  maxLev: 10,
  ...
  maxConcurrent: 2,
  maxOpenRiskPct: 4,
  maxHoldBars: 60, // 4H — 백테스트와 동일 (10일)
  barMs: 4 * 3600_000,
  dayMs: 24 * 3600_000,
  candleLimit: 300, // MACD(26+9) 시드 영향이 소멸하고 남을 길이 — SMA50 일봉도 덮는다.
  startEquity: 100,
};
...
function buildCtx(candles4h, daily) {
...
/** 4H 봉 i 시점에 "마감 완료된" 최신 일봉 인덱스 — 없으면 -1. */
function htfIdx(i, c) {
  let d = -1;
...
async function runCycle(client, state) {
  ...
  if (candles4h.length < 60 || daily.length < 60) {
    throw new Error(`캔들 부족(4H ${candles4h.length}·1D ${daily.length}) — 데이터 응답 이상`);
  }
  const last4h = candles4h[candles4h.length - 1];
  if (Date.now() > last4h.t + 2 * CFG.barMs + 30_000) summary.stale.push("4H");

  const ctx = buildCtx(candles4h, daily);

  // 열린 포지션 가상 체결 — 엔진 페이퍼 경로와 같은 보수 규칙.
  for (const [key, pos] of Object.entries({ ...state.positions })) {
    const bars = candles4h.filter((c) => c.t >= pos.entryTs);
  ...
  for (const [key, m] of Object.entries(CFG.members)) {
    const lastEval = state.lastBarTs[key] ?? candles4h[lastIdx - 1].t;
    for (let i = 1; i <= lastIdx; i += 1) {
      if (candles4h[i].t <= lastEval) continue;
      ...
      if (isLatest || fired) {
        summary.evaluated.push(
          `${key}@${new Date(candles4h[i].t).toISOString().slice(0, 16)} ${fired ? "신호" : "-"}${skip ? ` (${skip})` : ""}`,
        );
      }
    }
    state.lastBarTs[key] = candles4h[lastIdx].t;
  }
...
async function enter(client, state, key, m, ctx, i, summary) {
  ...
  const pos = {
    ...
    entryTs: ctx.candles[i].t + CFG.barMs,
    entryPrice: price,
    stop,
    target,
...
const TAG = "[CAND]";
console.log(`후보 전방 검증 (페이퍼·주문 없음) — ${Object.keys(CFG.members).join(", ")} · 리스크 ${CFG.riskPct}%`);
