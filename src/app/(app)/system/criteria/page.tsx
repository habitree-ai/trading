...
/**
 * 매매 기준 — 봇이 무엇을 보고 들어가고 나오는지의 사본.
 *
 * 정본은 `system-trading/docs/criteria.md` 이고 이 페이지는 사본이다 — 어긋나면 문서가 옳다.
 * 운용 현황(성적·포지션)과 나눠 둔 이유는 성격이 다르기 때문이다: 저쪽은 매 사이클
 * 바뀌는 실측이고, 여기는 파라미터를 바꾸기 전까지 움직이지 않는 규칙이다.
 */
...
const OPERATIONS: { label: string; desc: string }[] = [
  ...
  { label: "기록", desc: "모든 판정·주문·청산이 Supabase(진실 원천)에 남는다 — 봇이 어느 머신에서 돌든 운용 현황 화면에서 보인다" },
...
export default function SystemCriteriaPage() {
  return (
    <div className="space-y-6">
      <header>
        <h1 className="text-xl font-semibold tracking-tight">매매 기준 — 쿼드 공격형</h1>
        <p className="mt-1 text-sm text-dim">
          BTC-USDT-SWAP · 검증된 4개 기준의 병행 자동매매 · 정본은{" "}
          <code className="rounded bg-surface-2 px-1">system-trading/docs/criteria.md</code>
          {" · "}
          <Link href="/system" className="text-alpha">
            실제 성적은 운용 현황에서 →
          </Link>
        </p>
      </header>
          ...
          {CRITERIA.map((c) => (
            <div key={c.key} className="rounded-xl border border-border bg-surface p-4">
              ...
              <p className="mt-2 text-[12px]">
                <span className="text-dim">청산 </span>
                {c.exit}
              </p>
      ...
      <section className="rounded-xl border border-border bg-surface p-4">
        <h2 className="text-sm font-medium">
          병행 시 전체 그림 <span className="font-normal text-dim">— 쿼드 공격형, 리스크 10%</span>
        </h2>
        ...
        <p className="mt-3 rounded-lg border border-beta/40 bg-surface-2 p-3 text-[12px] text-dim">
          <b className="text-text">이 수치는 인샘플 상한이다.</b> 신호·청산·구성을 모두 같은
          2년에서 골랐고, 최고 t=1.74도 우연 기준선(≈1.9~2.5)을 넘지 못했다. 그래서 실전은 승격
          사다리를 따른다 — 지금은 소액 라이브 검증 구간이며, 운용 현황의 성과 데이터가 그 전방
          검증의 성적표다.
        </p>
      </section>
